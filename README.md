...
- **Multi-metric evaluation**:
  - Rule-based: Answer Accuracy 
  - DAG: Structural format comparison
  - BERTScore: Token-level semantic similarity (precision/recall/F1)
  - LLM grading: Qwen2.5-14B for Reasoning (from stripped reasoning) and Socratic Score (1–5) *(Suspended, see note below)*
...
2. **Fine-tune**:
   ...
   Output: `outputs/gsm8k_socratic_qwen/`
...
├── evaluate_rule_based.py          # Rule-based metrics
├── evaluate_bert.py                # BERTScore
├── evaluate_dag.py                 # DAG structure metrics
├── evaluate_with_qwen14b.py        # LLM-based grading
├── scripts/
...
├── notebook/
│   └── colab_run.ipynb             # Colab workflow
├── docs/
│   ├── END_TO_END_CHECKLIST.md
│   └── NEXT_STEPS_SOCRATIC.md
...
| Socratic Format (structure)| `evaluate_dag.py`        | Step/format/structural scores, duplicate detection |
...
- [End-to-end checklist](docs/END_TO_END_CHECKLIST.md)
- [Next steps & Socratic research notes](docs/NEXT_STEPS_SOCRATIC.md)

## Colab

Use [notebook/colab_run.ipynb](notebook/colab_run.ipynb) for a Google Colab workflow. Set `output_dir` in `socratic_train_config.yaml` to `/content/drive/MyDrive/llm_outputs/...` if saving checkpoints to Google Drive.
